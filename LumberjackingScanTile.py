...
def MoveToTree( spotnumber ):
    ...
    Misc.SendMessage('-->  X: %i - Y: %i - Z: %i' % (treeposx[spotnumber], treeposy[spotnumber], treeposz[spotnumber]), 77)
    PathFinding.PathFindTo(treeposx[spotnumber], treeposy[spotnumber], treeposz[spotnumber])(treeposx[spotnumber], treeposy[spotnumber], treeposz[spotnumber])
    while not RangeTree(spotnumber):
        CheckEnemy()  
        Misc.Pause(10)
        pathlock = pathlock + 1
        if pathlock > 350:
            Misc.SendMessage('PathFindTo X: %i - Y: %i - Z: %i' % (treeposx[spotnumber], treeposy[spotnumber], treeposz[spotnumber]), 77)
            PathFinding.PathFindTo(treeposx[spotnumber], treeposy[spotnumber], treeposz[spotnumber])  
            pathlock = 0
        else:
            Misc.Pause(5)
            Misc.SendMessage('pathlock < 350 : NoOperation')
            Misc.NoOperation()
