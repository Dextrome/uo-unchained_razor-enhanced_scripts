...
if sortItems:
    if not Misc.CheckSharedValue( 'itemsToKeepBox' ) or alwaysAskForContainers:
    ...
    if not Misc.CheckSharedValue( 'itemsToSellBox' ) or alwaysAskForContainers:
...
for item in containerToIdentify.Contains:
    ...
    if sortItems:
        ...
        if sortOnly:
            Misc.SendMessage("sort only")
            Journal.Clear()
            Misc.Pause(50)
            Items.SingleClick(item.Serial)
            Misc.Pause(200)
            if Journal.Search('Power') or Journal.Search('Vanquishing') or Journal.Search('Exceedingly') or Journal.Search('Supremely'):
                keepItem = True
        else:
